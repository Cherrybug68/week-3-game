...
var defaultImage = "assets/images/PlayIfYouDare.jpg";
...
//This captures key user selects
	...
	// All Letters guessed correctly, adds to # wins
		if (currentWord.indexOf("_") < 0) {
			wins++;
			//Displays image that matches random word
			document.getElementById("gameImage").src = imageOptions[wordOptions.indexOf(randomWord)];

		//Timeout function...resets after 3000 milliseconds (3 seconds)
			setTimeout(function(){ 

			// Resets initial values...all except wins		
				document.getElementById("gameImage").src = defaultImage;
	...
	// Didn't get entire word, guesses remaining equal zero		
		...
		//This only works if you do not immediately generate another word
			var randomWordArray = randomWord.split("");
			var randomWordHTML = randomWordArray.join(" ");

			// Links id code from HTML doc with JavaScript so it can be displayed on screen
				document.querySelector("#currentWord").innerHTML = randomWordHTML;
				document.getElementById("gameImage").src = imageOptions[wordOptions.indexOf(randomWord)];

		//Timeout function...reset after 3 seconds (3000 milliseconds)
			setTimeout(function(){
